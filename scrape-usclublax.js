...
// Netlify Function (ESM) — URL submission -> scrape -> upsert into Supabase
// Requires Node 18+ (fetch available). Protect with Authorization: Bearer <SCRAPE_API_TOKEN>
...
async function getOrCreateExternalSourceId(supabase) {
  // Uses your existing schema table: public.external_sources (name, base_url)
...
async function upsertTeamViaExternalLink(supabase, sourceId, externalId, externalUrl, teamName, gradYear) {
  // Find existing link
  ...
  if (!teamId) {
    // Create team without club mapping (club_id null); you can enrich later
...
async function upsertEvent(supabase, name, startDate) {
  // Uses public.events unique index on (lower(name), start_date) if present. If not, it still works but may duplicate.
  ...
  // Fallback when start_date is null and unique constraint doesn't fire
  ...
  if (!e2 && found?.length) return found[0].id;
  throw error;
}
...
async function touchQueueRow(supabase, url, status, lastError) {
  // Uses your schema: public.scrape_queue (url, status, last_run_at, last_error)
  // We'll insert-or-update by url (if you add a unique constraint later). For now: insert a row each run if not found.
  ...
  try {
    ...
    for (const url of urls.slice(0, 10)) {
      ...
      try {
        ...
        const summary = await upsertGames(supabase, teamId, externalTeamId, games);
        ...
        results.push({
          ...
          games_upserted: summary.upserted,
        });
      } catch (err) {
